...
from domainbed.utils.query import Q
from domainbed.dataset import datasets
from domainbed.utils import misc, reporting
from domainbed.learning import algorithms, model_selection
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", type=str, default="MAP/")
    ...
    results_file = "{}.tex".format(args.result_name) if args.latex else "{}.txt".format(args.result_name)
    
    args.result_dir = os.path.join(args.root, args.result_dir)
    args.input_dir = os.path.join(args.root, args.input_dir)
   
    sys.stdout = misc.Tee(os.path.join(args.result_dir, results_file), "w")
    ...
    if records and records[0]["args"].get("val_envs", False):
        ...
        SELECTION_METHODS = [
            model_selection.OracleSelectionMethodIIDMax,
            model_selection.OracleSelectionMethodOODMax,
            model_selection.OracleSelectionMethodIIDLast,
            model_selection.OracleSelectionMethodIID,
