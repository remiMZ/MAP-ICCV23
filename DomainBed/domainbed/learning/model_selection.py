...
class OracleSelectionMethodOODMax(SelectionMethod):
    """argmax(mean(test_out_acc_key for test_out_acc_key in test_envs))"""
    name = "max test-domain selection model (max OOD ACC)"
    
    @classmethod
    def _step_acc(self, record):
        test_env = record['args']['test_envs'][0]
        val_acc_keys= 'env{}_out_acc'.format(test_env)
        test_acc_key = 'env{}_in_acc'.format(test_env)
        return {
            'val_acc':  record[val_acc_keys],
...
class OracleSelectionMethodIIDMax(SelectionMethod):
    """argmax(mean(test_out_acc_key for test_out_acc_key in test_envs))"""
    name = "max test-domain selection model (max IID ACC)"
    ...
    def _step_acc(self, record):
        ...
        for i in itertools.count():
            if f'env{i}_out_acc' not in record:
                break
            if i != test_env:
                train_out_acc_keys.append(f'env{i}_out_acc')
        val_acc_keys = 'env{}_out_acc'.format(test_env)
        return {
            'val_acc': record[val_acc_keys],
            'test_acc': np.mean([record[key] for key in train_out_acc_keys])
        }
    
    @classmethod
    def _erm_step_acc(self, record):
        test_env = record['args']['test_envs'][0]
        train_out_acc_keys = []
        for i in itertools.count():
            if f'env{i}_out_acc' not in record:
        ...
        return {
            'val_acc': np.mean([record[key] for key in train_out_acc_keys]),
            'test_acc': np.mean([record[key] for key in train_out_acc_keys])
        }
    ...
    def run_acc(self, run_records):
        ...
        if test_records[0]['args']['algorithm'] == 'ERM':
            test_records = test_records.sorted(lambda r: r['step'])
            run_records = test_records[-1]
            return self._erm_step_acc(run_records)
        else:
            return test_records.map(self._step_acc).argmax('val_acc')
 
class OracleSelectionMethodIIDLast(SelectionMethod):
    """argmax(mean(test_out_acc_key for test_out_acc_key in test_envs))"""
    name = "last test-domain selection model (last IID ACC)"
    ...
    def run_acc(self, run_records):
        ...
        test_records = test_records.sorted(lambda r: r['step'])
        run_records = test_records[-1]
        return self._step_acc(run_records)
 
class OracleSelectionMethodIID(SelectionMethod):
    name = "train-domain validation set (oracle IID)"
    ...
    def run_acc(self, run_records):
        ...
        for i in range(10):
            ...
            if i != test_env:
                train_out_acc_keys.append(f'env{i}_out_acc')
                train_in_acc_keys.append(f'env{i}_in_acc')
                
        return {
            'val_acc':  np.mean([chosen_record[key] for key in train_out_acc_keys]),
            'test_acc': np.mean([chosen_record[key] for key in train_in_acc_keys])
        }   
